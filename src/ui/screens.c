...
static void event_handler_cb_main_watch(lv_event_t *e) {
    ...
    if (event == LV_EVENT_DRAW_PART_BEGIN) {
        ...
        temp = evalTextProperty(flowState, 25, 3, "Failed to evalute scale label in Meter widget");
        if (temp) {
...
void create_screen_main() {
    ...
    lv_obj_clear_flag(obj, LV_OBJ_FLAG_SCROLLABLE);
    {
        lv_obj_t *parent_obj = obj;
        {
            ...
            lv_obj_set_style_bg_color(obj, lv_color_hex(0xffffa207), LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_bg_grad_color(obj, lv_color_hex(0xff8d3178), LV_PART_MAIN | LV_STATE_DEFAULT);
            {
                lv_obj_t *parent_obj = obj;
                {
                    // exit_btn
...
void tick_screen_main() {
    void *flowState = getFlowState(0, 0);
    {
        ...
        if (indicator) {
            int32_t new_val = evalIntegerProperty(flowState, 23, 3, "Failed to evaluate Value in Meter widget");
    ...
    }
    {
        ...
        if (indicator) {
            int32_t new_val = evalIntegerProperty(flowState, 24, 3, "Failed to evaluate End value in Meter widget");
    ...
    }
    {
        ...
        if (indicator) {
            int32_t new_val = evalIntegerProperty(flowState, 24, 4, "Failed to evaluate End value in Meter widget");
            int32_t cur_val = indicator->end_value;
            if (new_val != cur_val) {
                tick_value_change_obj = objects.arcs;
                lv_meter_set_indicator_end_value(objects.arcs, indicator, new_val);
    ...
    }
    {
        ...
        if (indicator) {
            int32_t new_val = evalIntegerProperty(flowState, 24, 5, "Failed to evaluate End value in Meter widget");
            int32_t cur_val = indicator->end_value;
            if (new_val != cur_val) {
                tick_value_change_obj = objects.arcs;
                lv_meter_set_indicator_end_value(objects.arcs, indicator, new_val);
    ...
    }
    {
        ...
        if (indicator) {
            int32_t new_val = evalIntegerProperty(flowState, 25, 4, "Failed to evaluate Value in Meter widget");
            int32_t cur_val = indicator->start_value;
            if (new_val != cur_val) {
    ...
    }
    {
        ...
        if (indicator) {
            int32_t new_val = evalIntegerProperty(flowState, 25, 5, "Failed to evaluate Value in Meter widget");
            int32_t cur_val = indicator->start_value;
            if (new_val != cur_val) {
                tick_value_change_obj = objects.watch;
                lv_meter_set_indicator_value(objects.watch, indicator, new_val);
    ...
    }
    {
        ...
        if (indicator) {
            int32_t new_val = evalIntegerProperty(flowState, 25, 6, "Failed to evaluate Value in Meter widget");
            int32_t cur_val = indicator->start_value;
            if (new_val != cur_val) {
                tick_value_change_obj = objects.watch;
                lv_meter_set_indicator_value(objects.watch, indicator, new_val);
